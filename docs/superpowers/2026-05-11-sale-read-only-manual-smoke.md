...
| Date | Commit | Change |
|---|---|---|
| 2026-05-13 | Push harden (this commit) | Manual Create dialog switched from `GET /api/customers?search=` (full PII payload) → new sale-scoped `GET /api/sale/customers/search?q=` returning only `customerId / name / phone / email / isBanned / orderCount`. Forbidden fields (address / labels / notes / channel / facebookId / bannedReason / shopId / timestamps) no longer cross the wire. Route + 22 RBAC/validation/PII-whitelist tests added. idempotencyKey lifecycle documented + commented (stable across retry inside one draft, regenerates on close+reopen only). Mutation surface unchanged (4 POSTs). |
| 2026-05-13 | Manual Create Phase 4 | Submit enabled on `ManualCreateBookingDialog`. POST `/api/sale/bookings` fires with auto-generated `idempotencyKey` (UUID v4) + status locked to `PENDING_REVIEW`. 8-case error mapping (401/403/400/404/409/422/429/500) mirrors Confirm/Cancel/CreateOrder dialogs. Success toast + close + parent refetch. Mutation surface = **4** intentional POSTs. |
