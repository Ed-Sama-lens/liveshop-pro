...
| Date | Commit | Change |
|---|---|---|
| 2026-05-11 | 2O-b (this commit) | Cancel button enabled (single-row, CONFIRMED + integrity-clean). Required reason 3-200 chars. targetStatus hard-coded CANCELLED. Mutation surface = 2 intentional POSTs (Confirm + Cancel). |
...
- ✅ Create Order + Manual Create still disabled.
...
- 2O-c Create Order multi-select (larger scope; needs row multi-select state model)
